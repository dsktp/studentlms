...
const handleEdite=(user)=>{
  ...
  const handleUpdate=async(user)=>{
    console.log(user)
await setDoc(doc(firestore, "users", users.id), user ,{merge: true });
console.log("document updated")
// let newUsers=user.map((oldUser)=>{
...
<div className="container">
  <div className="row">
    <div className='col'>
      ...
      <Link to='/Add' element={<AddUser/>}  className='btn btn-success item-end  '>Add New Student</Link>
      <hr />
      { users.length>0
      ?
   ...
   <table class="table w-100 table-striped">
     ...
     <td>
       <button className='btn btn-info me-3'data-bs-toggle="modal" data-bs-target="#exampleModal" onClick={()=>{handleEdite(user)}}>Update</button>
